...
def build_parser() -> argparse.ArgumentParser:
  ...
  parser.add_argument("--start", required=False, default=None, help="Start time (ISO 8601 or 'YYYY-MM-DD HH:MM:SS'). UTC assumed if naive. If omitted, uses state watermark or last 24h")
  parser.add_argument("--end", required=False, default=None, help="End time (ISO 8601 or 'YYYY-MM-DD HH:MM:SS'). UTC assumed if naive. Defaults to now if omitted")
  ...
  parser.add_argument("--state-table", default=None, help="Optional state table (catalog.schema.table) to persist and read last_run_iso watermark")
